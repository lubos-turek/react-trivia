...
const Results = () => {
  ...
  const score = latestResults.filter(q => q.userAnswer === q.correctAnswer).length
  const questionsAmount = latestResults.length

  return (
    <>
      <h1>You Scored<br />{score} / {questionsAmount}</h1>
      { latestResults.map( (question, i) => <p key={i}>{question.userAnswer === question.correctAnswer ? '+' : '-'} {question.question}</p>) }
